...
public class TestPersonController {
	...
	public void testCreateNewPerson() {
		given().
			param("name","pepe").
			param("age",10).
			param("description","hace popo").
		when().
			post("/rest/person/create").
		then().
			statusCode(200).
			body("id",equalTo(1));
	}
	...
	public void testGetPersonById(){
		when().
			get("/rest/person/get/{id}",0).
		then().
			statusCode(200).
			body("name",equalTo("John 0")).
			body("age",equalTo(20)).
	...
	public void testGetListOfPersons() {
		...
		then().
			statusCode(200).
			body("name[0]",equalTo("John 0")).
			body("age[0]",equalTo(20)).
			body("description[0]",equalTo("description 0")).
			body("name[1]",equalTo("John 1")).
			body("age[1]",equalTo(20)).
			body("description[1]",equalTo("description 1")).
			body("name[2]",equalTo("John 2")).
	...
	public void testDeletePersonById() {
		...
		when().
			delete("/rest/person/delete/{id}",1).
		then().
			statusCode(200).
			body("name",equalTo("John 1")).
			body("age",equalTo(20)).
	...
	public void testUpdatePerson() {
		given().
			...
			body(new Person(1,"Juan",90,"New description")).
		when().
			put("/rest/person/update").
		then().
			statusCode(200);
	}
}